...
<aside class="{appState.sidebarCollapsed ? 'w-20' : 'w-64'} bg-card border-r border-border h-screen flex flex-col fixed left-0 top-0 z-50 transition-all duration-300 ease-in-out">
  <!-- Header -->
  <div class="h-14 flex items-center {appState.sidebarCollapsed ? 'justify-center' : 'px-4'} border-b border-border relative">
    <button
    ...
    >
      ...
      {#if !appState.sidebarCollapsed}
        <span class="font-bold text-lg tracking-tight text-foreground whitespace-nowrap overflow-hidden">SwissKit</span>
      {/if}
    </button>
  </div>
  ...
  <div class="flex-1 overflow-y-auto py-4 px-3 space-y-6">
    ...
    {#each categories as category}
      ...
      {#if categoryTools.length > 0}
        <div>
          {#if !appState.sidebarCollapsed}
            <h3 class="px-3 text-xs font-semibold text-muted-foreground uppercase tracking-wider mb-2 truncate">
              {category}
            </h3>
          {/if}
          <div class="space-y-1">
  ...
  <!-- Toggle Button -->
  <div class="p-4 border-t border-border flex {appState.sidebarCollapsed ? 'justify-center' : 'justify-end'}">
    <button
      onclick={toggleSidebar}
      class="p-2 hover:bg-accent rounded-lg text-muted-foreground hover:text-foreground transition-colors"
